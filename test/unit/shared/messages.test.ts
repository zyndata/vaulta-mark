...
describe('parseRequest', () => {
  it('accepts every payload-free request', () => {
    for (const type of ['PING', 'GET_STATE', 'TOUCH', 'GET_SETTINGS'] as const) {
...
describe('parseResponse / parseBroadcast', () => {
  it('recognises every response type', () => {
    for (const type of ['PONG', 'STATE', 'OK', 'SETTINGS', 'TOUCHED', 'ERROR']) {
  ...
  it('recognises every broadcast type and nothing else', () => {
    for (const type of ['SESSION_LOCKED', 'SESSION_UNLOCKED', 'SETTINGS_CHANGED']) {
